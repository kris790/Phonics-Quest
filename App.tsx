...
import { RootState, BattleState, AnimationState, PhonicsTask, AppState, Chapter, Attributes, Quest, ActivityEntry, Artifact, GameSettings, Pet, Egg, BattleRewards, LootItem } from './types';
...
import { CHAPTERS, NPCS, ASSETS, FORGE_RECIPES } from './constants';
...
type Action = 
  ...
  | { type: 'COMPLETE_CHAPTER'; chapterId: string; artifact?: Artifact; rewards?: BattleRewards }
  ...
  | { type: 'FORGE_ITEM'; recipeId: string }
  | { type: 'EQUIP_ITEM'; itemId: string; slot: 'weapon' | 'armor' | 'charm' }
  | { type: 'CHECK_STREAK' };
...
function rootReducer(state: RootState, action: Action): RootState {
  switch (action.type) {
    ...
    case 'EQUIP_ITEM': {
      const gear = { ...state.progression.equippedGear };
      if (gear[action.slot] === action.itemId) {
        delete gear[action.slot];
      } else {
        gear[action.slot] = action.itemId;
      }
      return { ...state, progression: { ...state.progression, equippedGear: gear } };
    }
    case 'FORGE_ITEM': {
      const recipe = FORGE_RECIPES.find(r => r.id === action.recipeId);
      if (!recipe) return state;
      const logs = [...state.progression.recentActivities];
      logs.push({ id: generateSmartId('forge'), type: 'artifact_forged', description: `Forged ${recipe.result.name}!`, timestamp: Date.now() });
      const newInventory = [...state.progression.inventory, { ...recipe.result, id: `item-${Date.now()}` }];
      let newCrystals = state.progression.crystalsFound - recipe.costs.crystals;
      return { ...state, progression: { ...state.progression, crystalsFound: newCrystals, inventory: newInventory, recentActivities: logs.slice(-20) } };
    }
    case 'DAILY_SPIN': {
      const logs = [...state.progression.recentActivities];
    ...
    case 'HATCH_EGG': {
        ...
        return { ...state, progression: { ...state.progression, activeEggs: state.progression.activeEggs.filter(e => e.id !== action.eggId), pets: [...state.progression.pets, action.pet], recentActivities: logs.slice(-20) } };
    }
    case 'SET_ACTIVE_PET':
    ...
    case 'DISMISS_DISCLAIMER':
      return { ...state, progression: { ...state.progression, hasSeenDisclaimer: true } };
    case 'ADD_XP': {
      ...
      while (newXp >= newMaxXp) {
        ...
        logs.push({ id: generateSmartId('level-up'), type: 'level_up', description: `Ascended to Power Level ${newLevel}!`, timestamp: Date.now() });
      }
      return { ...state, progression: { ...state.progression, xp: newXp, level: newLevel, maxXp: newMaxXp, recentActivities: logs.slice(-20) } };
    }
    case 'ADD_CRYSTALS':
    ...
    case 'COMPLETE_CHAPTER': {
      ...
      if (nextChapterIdx < newChapters.length) newChapters[nextChapterIdx] = { ...newChapters[nextChapterIdx], isUnlocked: true };
      const newNPC = NPCS.find(n => n.unlockedAfter === action.chapterId);
      const newUnlockedNPCs = newNPC && !state.progression.unlockedNPCs.includes(newNPC.id) ? [...state.progression.unlockedNPCs, newNPC.id] : state.progression.unlockedNPCs;
      const logs = [...state.progression.recentActivities];
      ...
      const newInventory = [...state.progression.inventory, ...(action.rewards?.loot || [])];
      ...
      return { ...state, chapters: newChapters, progression: { ...state.progression, unlockedNPCs: newUnlockedNPCs, restorationPoints: state.progression.restorationPoints + 100, artifacts: action.artifact ? [...state.progression.artifacts, action.artifact] : state.progression.artifacts, inventory: newInventory, activeEggs: newEggs, recentActivities: logs.slice(-20) } };
    }
    case 'UPGRADE_ATTRIBUTE':
      return { ...state, progression: { ...state.progression, crystalsFound: state.progression.crystalsFound - action.cost, attributes: { ...state.progression.attributes, [action.attribute]: state.progression.attributes[action.attribute] + 1 } } };
    case 'CHECK_STREAK': {
      const now = Date.now();
      const last = state.progression.lastLoginDate;
      if (!last) return { ...state, progression: { ...state.progression, lastLoginDate: now, dailyStreak: 1 } };
      const diff = now - last;
      const oneDay = 86400000;
      const thirtySixHours = 129600000;
      if (diff > thirtySixHours) return { ...state, progression: { ...state.progression, lastLoginDate: now, dailyStreak: 1 } };
      else if (diff >= oneDay) return { ...state, progression: { ...state.progression, lastLoginDate: now, dailyStreak: state.progression.dailyStreak + 1 } };
      return { ...state, progression: { ...state.progression, lastLoginDate: now } };
    }
    case 'RESET_GAME':
      localStorage.removeItem('phonics_quest_save_v5_armor');
      return { ...initialState };
    case 'LOAD_SAVE':
      return { 
        ...state, 
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (isStarted) dispatch({ type: 'CHECK_STREAK' });
  }, [isStarted]);
  ...
  const currentChapter = useMemo(() => state.chapters.find(c => c.id === state.currentChapterId) || state.chapters[0], [state.chapters, state.currentChapterId]);

  const getModifiedAttributes = useCallback(() => {
    const attrs = { ...state.progression.attributes };
    const gearSlots: (keyof typeof state.progression.equippedGear)[] = ['weapon', 'armor', 'charm'];
    gearSlots.forEach(slot => {
      const itemId = state.progression.equippedGear[slot];
      const item = state.progression.inventory.find(i => i.id === itemId);
      if (item?.stats) {
        Object.keys(item.stats).forEach(k => {
          const key = k as keyof Attributes;
          attrs[key] += item.stats![key] || 0;
        });
      }
    });
    if (activePet?.buffType === 'damage_reduction') attrs.resilience += 5;
    if (activePet?.buffType === 'crit_boost') attrs.focus += 10;
    return attrs;
  }, [state.progression.attributes, state.progression.equippedGear, state.progression.inventory, activePet]);
  ...
  const handleSelect = useCallback(async (selected: string) => {
    ...
    const attrs = getModifiedAttributes();
    const { nextState } = BattleEngine.processTurn(state.battle, isCorrect, attrs);
    ...
    if (isCorrect) {
      ...
      const guardianState = BattleEngine.processGuardianTurn(nextState, attrs.resilience);
      setTimeout(() => {
  ...
  }, [state.battle, getModifiedAttributes, state.progression.settings.ttsVoice]);

  const voiceTimeout = activePet?.buffType === 'timeout_extension' ? state.progression.settings.voiceTimeout + 2000 : state.progression.settings.voiceTimeout;
  const { status: voiceStatus, start: startVoice, stop: stopVoice } = useVoiceSession(
    process.env?.API_KEY || "", 
    mappedGameState.currentQuestion?.correctDigraph,
    voiceTimeout,
    handleSelect
  );
  ...
  const startGame = async () => {
    try {
      ...
      console.warn("Audio context resume failed, proceeding anyway", e);
    }
    setIsStarted(true);
  };
  ...
  const startChapter = async (chapter: Chapter) => {
    ...
    try {
      ...
      const tasks: PhonicsTask[] = questions.map((q, i) => ({ ...q, taskId: `task-${i}-${Date.now()}` }));
      dispatch({ type: 'INIT_BATTLE', tasks, chapter });
    } catch (e) {
      setToast({ message: "Echoes unstable. Try again.", type: 'error' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleBattleEnd = (victory: boolean, rewards?: BattleRewards | null, artifact?: Artifact) => {
    if (victory) {
      ...
      dispatch({ type: 'COMPLETE_CHAPTER', chapterId: state.currentChapterId, artifact, rewards: actualRewards });
    }
    dispatch({ type: 'SET_VIEW', view: 'world-map' });
  };

  return (
    <div className="relative mx-auto max-w-[430px] h-screen max-h-[932px] overflow-hidden flex flex-col bg-background-dark" onClick={() => resumeAudioContext()}>
      ...
      {!isStarted ? (
        <div className="absolute inset-0 z-[200] bg-background-dark flex flex-col items-center justify-center p-8 text-center animate-fadeIn">
            ...
            <button onClick={startGame} className="px-12 py-5 bg-primary text-background-dark rounded-2xl font-black uppercase tracking-widest text-sm shadow-[0_0_40px_rgba(13,223,242,0.4)] active:scale-95 transition-all">Awaken the Kingdom</button>
        </div>
      ) : (
        <>
          ...
          <div className="flex-1 flex flex-col h-full relative z-10">
            <HUD gameState={mappedGameState} battleState={state.battle} progression={state.progression} onReset={() => dispatch({ type: 'SET_VIEW', view: 'world-map' })} isMuted={isMuted} onToggleMute={() => dispatch({ type: 'UPDATE_SETTINGS', updates: { isMuted: !isMuted } })} onOpenSettings={() => dispatch({ type: 'SET_VIEW', view: 'settings' })} />
            ...
            <div className="flex-1 relative overflow-hidden">
              {state.view === 'world-map' && (
                <div className="absolute inset-0 z-40 bg-background-dark p-6 overflow-y-auto custom-scrollbar animate-fadeIn">
                  <header className="mb-6"><h1 className="text-3xl font-black text-white italic uppercase tracking-tighter">Kingdom Map</h1></header>
                  {state.chapters.map(ch => (
                    <div key={ch.id} onClick={() => ch.isUnlocked && startChapter(ch)} className={`relative overflow-hidden rounded-2xl border mb-4 transition-all ${ch.isUnlocked ? 'border-primary/20 hover:border-primary cursor-pointer' : 'opacity-50 grayscale border-white/5'}`}>
                ...
                <>
                  ...
                  {state.battle.phase !== 'victory' && state.battle.phase !== 'defeat' && (
                    <Overlay 
                      ...
                      onPronounce={() => playTTS(mappedGameState.currentQuestion?.word || "")} 
                      onVoiceStart={startVoice} 
                      voiceStatus={voiceStatus} 
                      onDismissDisclaimer={() => dispatch({ type: 'DISMISS_DISCLAIMER' })}
                      onDebugVictory={() => dispatch({ type: 'INSTANT_VICTORY' })} 
                    />
                  )}
                </>
              )}
              {state.view === 'sanctuary' && <Sanctuary progression={state.progression} onClaimReward={() => {}} onViewLedger={() => dispatch({ type: 'SET_VIEW', view: 'ledger' })} onOpenParentDashboard={() => dispatch({ type: 'SET_VIEW', view: 'parent-dashboard' })} onSpin={(type, r, a) => dispatch({ type: 'DAILY_SPIN', rewardType: type, rarity: r, amount: a })} onHatch={(id, pet) => dispatch({ type: 'HATCH_EGG', eggId: id, pet })} onForge={(rid) => dispatch({ type: 'FORGE_ITEM', recipeId: rid })} />}
              {state.view === 'hero-room' && <HeroRoom progression={state.progression} onEquipDecoration={(slot, id) => dispatch({ type: 'EQUIP_DECORATION', slot, decorationId: id })} onSetActivePet={(id) => dispatch({ type: 'SET_ACTIVE_PET', petId: id })} onEquipItem={(id, slot) => dispatch({ type: 'EQUIP_ITEM', itemId: id, slot })} />}
            ...
            <nav className="relative z-50 h-16 bg-background-dark/95 border-t border-white/5 flex items-center justify-around px-4">
              {['world-map', 'sanctuary', 'hero-room', 'quest-log', 'character-sheet'].map(id => (
                <button key={id} onClick={() => dispatch({ type: 'SET_VIEW', view: id as any })} className={`material-symbols-outlined transition-colors p-2 rounded-lg ${state.view === id ? 'text-primary' : 'text-white/40'}`}>
                  {id === 'world-map' ? 'map' : id === 'sanctuary' ? 'castle' : id === 'hero-room' ? 'home' : id === 'quest-log' ? 'assignment' : 'person'}
                </button>
              ))}
            </nav>
          </div>
          ...
          {/* Tutorial Flow */}
          {!state.progression.hasSeenTutorial && (
